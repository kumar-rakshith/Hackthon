...
const AdminDashboardLayout = ({ children }) => {
  ...
  const [batch, setBatch] = useState([]); // Departments state

  // Admin Info (mocked data)
  const adminInfo = {
    name: "MCA Faculty",
    profilePicture: "https://randomuser.me/api/portraits/female/75.jpg",
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/faculty-batch"); // Replace with your backend URL
        setBatch(response.data); // Store the fetched data in state
      } catch (error) {
  ...
  const handleDepartmentClick = (batch) => {
    setIsDropdownOpen(false);
    navigate(`/faculty/dashboard/${batch.batch_name}`);
  };
        ...
        <div className="w-64 bg-white shadow-lg py-6 px-4 flex flex-col space-y-6">
          ...
          >
            <Link
              ...
              className={`block text-center w-full py-2 rounded-md ${isActive("/faculty/dashboard")}`}
            >
              Batch
            </Link>
                ...
                <ul>
                  {batch.map((bat, index) => (
                    <li
                      ...
                      onClick={() => handleDepartmentClick(bat)}
                    >
                      <h2 className="text-sm font-semibold">{bat.batch_name}</h2>
                    </li>
                  ))}
                </ul>
              </div>
